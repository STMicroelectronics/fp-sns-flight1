...
static void Init_ToF_Sensor(void)
{
  ...
  RANGING_SENSOR_SET_POWER_MODE(TOF_INSTANCE, POWERMODE_OFF);
  /* Power on center device */
  RANGING_SENSOR_SET_POWER_MODE(TOF_INSTANCE, POWERMODE_ON);
  ...
  else
  {
    ...
    i2c_addr = (TOF_RANGING_SENSOR_ADDRESS + (TOF_INSTANCE + 1) * 2);
    RANGING_SENSOR_SET_ADDRESS(TOF_INSTANCE, i2c_addr);
